...
const HistoryBar = ({ setEditingTranslation, setSelectedTranslation }) => {
  const [translations, setTranslations] = useState([]);
    ...
    <div style={{ fontFamily: 'Nunito' }} className='bg-gray-200 h-screen p-4 w-72'>
      <div className="mx-auto">
        ...
        {translations.map((translation) => (
          <div key={translation._id} className="mb-2 flex bg-[#FFAF45] rounded-xl px-4">
            <div>
              <p className="text-sm font-semibold">{`${translation.title}...`}</p>
              <p className="text-gray-600">{translation.translatedText}</p>
            </div>
            <div className="flex justify-end items-end">
              <button onClick={() => handleEdit(translation)} className='mr-2'><EditIcon /></button>
              <button onClick={() => deleteTranslation(translation._id)}><DeleteForeverTwoToneIcon /></button>
            </div>
          </div>
        ))}
      </div>
